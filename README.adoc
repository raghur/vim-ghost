...
* [line-through]#Install python websocket server `sudo pip install git+https://github.com/dpallot/simple-websocket-server.git`#
** [line-through]#If you don't want to install system wide, then just pass `--user` in the command above.#
** This is now done automatically during installation with `:GhostInstall`
